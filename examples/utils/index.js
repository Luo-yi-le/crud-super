...
export function renderNode(vnode, options = {}) {
  ...
  if (isFunction(vnode)) {
    ...
    if (vnode.includes("slot-")) {
      let rn = this.crud
        ? this.crud.$scopedSlots[vnode]
        : this.$scopedSlots[vnode];
      ...
      if (isFunction(e.component)) {
        ...
        if (vm && isFunction(vm)) {
          ...
          if (name == "el-button") {
            ...
            if (viewing) {
              const value = make.format(
                { h, scope, nextTick: this.$nextTick, child: children },
                e,
                jsx
              );
          ...
          <template {...{ slot: "label" }}>
            {(() => {
              if (typeof e.label === "function") {
                return e.label({ scope, view: viewing, edit: isEdit });
              } else {
                return e.label;
              }
            })()}
